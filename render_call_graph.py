...
def print_ascii_tree(node, indent="", is_last=True):
    """Recursively prints the call graph as an ASCII tree."""
...
def export_to_dot(root_node, dot_filepath):
    """Exports the tree to Graphviz DOT format for SVG/PNG rendering."""
    edges = set()
    nodes = set()

    def collect_edges(node):
        parent_id = f"{node.get('className')}.{node.get('methodName')}"
        nodes.add((parent_id, node.get('methodName'), node.get('className').split('.')[-1], node.get('status')))
    ...
    with open(dot_filepath, "w", encoding="utf-8") as f:
        ...
        for node_id, method, short_cls, status in nodes:
            ...
            elif status == "circular_dependency_detected":
